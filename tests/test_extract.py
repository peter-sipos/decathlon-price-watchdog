...
def first_url(item: dict, url: str) -> str:
    """The cheapest tier of the item's proxy ladder."""
...
class FetchTests(unittest.TestCase):
    """A bot-check body must fail the fetch; a hiccup gets exactly one retry."""

    def setUp(self):
        self.calls = []
        self.addCleanup(setattr, check_prices, "fetch_urllib", check_prices.fetch_urllib)

    def install(self, *behaviours):
        queue = list(behaviours)

        def fake(url, accept_language, timeout):
        ...
        check_prices.fetch_urllib = fake

    def test_returns_a_real_page(self):
        good = "<html>" + "product " * 500 + "</html>"
        self.install(good)
        self.assertEqual(fetch("https://example.test/p", "cs-CZ"), good)
        self.assertEqual(len(self.calls), 1)

    def test_bot_check_body_counts_as_failure_not_success(self):
        blocked = "<html><h1>Access Denied</h1>" + "x" * 3000 + "</html>"
        self.install(blocked)
    ...
    def test_a_rejection_is_not_retried(self):
        """Retrying an HTTP 403 spends a second scraping credit for the same answer."""
        self.install(FetchError("HTTP 403", "<h1>Access Denied</h1>"))
        with self.assertRaises(FetchError):
            fetch("https://example.test/p", "cs-CZ")
        self.assertEqual(len(self.calls), 1)

    def test_a_connection_failure_is_retried_once(self):
        good = "<html>" + "product " * 500 + "</html>"
        self.install(FetchError("URLError: timed out", transient=True), good)
        with mock.patch.object(check_prices.time, "sleep"):
            self.assertEqual(fetch("https://example.test/p", "cs-CZ"), good)
        self.assertEqual(len(self.calls), 2)

    def test_the_failed_body_is_written_for_the_artifact(self):
        self.install(FetchError("HTTP 403", "<h1>Access Denied</h1>"))
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaises(FetchError):
                fetch("https://example.test/p", "cs-CZ", debug_dir=Path(tmp), slug="cz")
            self.assertEqual(
                [p.name for p in Path(tmp).iterdir()], ["cz.failed.html"]
            )
...
class ProxyTemplateTests(unittest.TestCase):
    ...
    URL = "https://www.decathlon.cz/p/polstar-ultim-comfort/_/R-p-308736"
    ...
    def test_no_template_returns_url_unchanged(self):
        self.assertEqual(first_url({}, self.URL), self.URL)
    ...
    def test_key_and_encoded_url_are_substituted(self):
        ...
        built = first_url(item, self.URL)
        self.assertIn("api_key=secret123", built)
        self.assertIn("https%3A%2F%2Fwww.decathlon.cz", built)
        self.assertNotIn("/p/", built.split("url=")[1])  # fully encoded, not raw

    def test_missing_key_fails_loudly(self):
        item = {"proxy_template": "https://api.example.com/?api_key={key}&url={url_encoded}"}
        with self.assertRaises(FetchError) as caught:
            first_url(item, self.URL)
    ...
    def test_custom_key_env(self):
        ...
        self.assertEqual(first_url(item, self.URL), "https://x/?k=abc")

    def test_key_is_redacted_from_text(self):
        os.environ["SCRAPER_API_KEY"] = "supersecret"
        first_url({"proxy_template": "https://x/?k={key}"}, self.URL)
...
class ProxyEscalationTests(unittest.TestCase):
    ...
    def test_the_first_url_is_the_cheapest_tier(self):
        item = {"proxy_templates": ["https://cheap/?k={key}", "https://dear/?k={key}"]}
        self.assertEqual(first_url(item, self.URL), "https://cheap/?k=k")
